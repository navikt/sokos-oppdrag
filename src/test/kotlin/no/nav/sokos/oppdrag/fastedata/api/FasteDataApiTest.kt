...
internal class FasteDataApiTest : FunSpec({
    ...
    test("hent fagområder returnerer 200 OK") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/fagomraader").then().assertThat().statusCode(HttpStatusCode.OK.value).extract().response()
    ...
    test("hent fagområder returnerer 500 Internal Server Error") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/fagomraader").then().assertThat().statusCode(HttpStatusCode.InternalServerError.value).extract().response()
    ...
    test("korrigeringsårsaker tilhørende fagområde returnerer 200 OK") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/fagomraader/MYSTB/korrigeringsaarsaker").then().assertThat().statusCode(HttpStatusCode.OK.value).extract().response()
    ...
    test("korrigeringsårsaker tilhørende fagområde returnerer 500 Internal Server Error") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/fagomraader/''!/korrigeringsaarsker").then().assertThat().statusCode(HttpStatusCode.InternalServerError.value).extract().response()
    ...
    test("korrigeringsårsaker tilhørende fagområde med ugyldig query parameter returnerer 400 Bad Request") {
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/fagomraader/''!/korrigeringsaarsaker").then().assertThat().statusCode(HttpStatusCode.BadRequest.value).extract().response()
    ...
    test("hent billagstyper returnerer 200 OK") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/billagstyper").then().assertThat().statusCode(HttpStatusCode.OK.value).extract().response()
    ...
    test("hent billagstyper returnerer 500 Internal Server Error") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/billagstyper").then().assertThat().statusCode(HttpStatusCode.InternalServerError.value).extract().response()
    ...
    test("hent klassekoder returnerer 200 OK") {
        val mockedKlassekoder =
            listOf(
                Klassekode(kodeKlasse = "KLASSE1"),
                Klassekode(kodeKlasse = "KLASSE2"),
            )
        coEvery { fasteDataService.getKlassekoder(any()) } returns mockedKlassekoder

        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/klassekoder").then().assertThat().statusCode(HttpStatusCode.OK.value).extract().response()

        val actualKlassekoder: List<Klassekode> = Json.decodeFromString(response.asString())

        actualKlassekoder shouldBe mockedKlassekoder
    }
    ...
    test("hent klassekoder returnerer 500 Internal Server Error") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/klassekoder").then().assertThat().statusCode(HttpStatusCode.InternalServerError.value).extract().response()

        val apiErrorResponse = Json.decodeFromString<ApiError>(response.asString())

        apiErrorResponse.status shouldBe HttpStatusCode.InternalServerError.value
        apiErrorResponse.message shouldBe "En feil"
        apiErrorResponse.path shouldBe "$FASTEDATA_BASE_API_PATH/klassekoder"
        apiErrorResponse.timestamp shouldNotBe null
    }
    ...
    test("ventekriterier returnerer 200 OK") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/ventekriterier").then().assertThat().statusCode(HttpStatusCode.OK.value).extract().response()
    ...
    test("ventekriterier returnerer 500 Internal Server Error") {
        ...
        val response =
            RestAssured.given().filter(validationFilter).header(HttpHeaders.ContentType, APPLICATION_JSON).header(HttpHeaders.Authorization, "Bearer $tokenWithNavIdent").port(PORT)
                .get("$FASTEDATA_BASE_API_PATH/ventekriterier").then().assertThat().statusCode(HttpStatusCode.InternalServerError.value).extract().response()
