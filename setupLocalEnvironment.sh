...
envValue=$(kubectl exec -it $(kubectl get pods | grep sokos-oppdrag | cut -f1 -d' ') -c sokos-oppdrag -- env | egrep "^AZURE|^DATABASE|EREG_URL|TP_URL|^PDL|^SKJERMING|^GA_OKONOMI|^GA_SOKOS")
